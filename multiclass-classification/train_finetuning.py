...
from utils_dense_vp_sampling import quaternion_cross_entropy_aero
from utils_dense_vp_sampling import quaternion_cross_entropy_bike
from utils_dense_vp_sampling import quaternion_cross_entropy_boat
from utils_dense_vp_sampling import quaternion_cross_entropy_bottle
from utils_dense_vp_sampling import quaternion_cross_entropy_bus
from utils_dense_vp_sampling import quaternion_cross_entropy_car
from utils_dense_vp_sampling import quaternion_cross_entropy_chair
from utils_dense_vp_sampling import quaternion_cross_entropy_table
from utils_dense_vp_sampling import quaternion_cross_entropy_mbike
from utils_dense_vp_sampling import quaternion_cross_entropy_sofa
from utils_dense_vp_sampling import quaternion_cross_entropy_train
from utils_dense_vp_sampling import quaternion_cross_entropy_tv
...
df_aero = pd.read_csv(data_dir + f"Image_sets/aeroplane/aeroplane_train_cls_{args.samples}.csv", sep=",")
df_aero = df_aero.sample(frac=1)
df_bike = pd.read_csv(data_dir + f"Image_sets/bicycle/bicycle_train_cls_{args.samples}.csv", sep=",")
df_bike = df_bike.sample(frac=1)
df_boat = pd.read_csv(data_dir + f"Image_sets/boat/boat_train_cls_{args.samples}.csv", sep=",")
df_boat =  df_boat.sample(frac=1)
df_bottle = pd.read_csv(data_dir + f"Image_sets/bottle/bottle_train_cls_{args.samples}.csv", sep=",")
df_bottle = df_bottle.sample(frac=1)
df_bus = pd.read_csv(data_dir + f"Image_sets/bus/bus_train_cls_{args.samples}.csv", sep=",")
df_bus =  df_bus.sample(frac=1)
df_car = pd.read_csv(data_dir + f"Image_sets/car/car_train_cls_{args.samples}.csv", sep=",")
df_car = df_car.sample(frac=1)
df_chair = pd.read_csv(data_dir + f"Image_sets/chair/chair_train_cls_{args.samples}.csv", sep=",")
df_chair = df_chair.sample(frac=1)
df_table = pd.read_csv(data_dir + f"Image_sets/diningtable/diningtable_train_cls_{args.samples}.csv", sep=",")
df_table = df_table.sample(frac=1)
df_mbike = pd.read_csv(data_dir + f"Image_sets/motorbike/motorbike_train_cls_{args.samples}.csv", sep=",")
df_mbike = df_mbike.sample(frac=1)
df_sofa = pd.read_csv(data_dir + f"Image_sets/sofa/sofa_train_cls_{args.samples}.csv", sep=",")
df_sofa = df_sofa.sample(frac=1)
df_train = pd.read_csv(data_dir + f"Image_sets/train/train_train_cls_{args.samples}.csv", sep=",")
df_train =  df_train.sample(frac=1)
df_tv = pd.read_csv(data_dir + f"Image_sets/tvmonitor/tvmonitor_train_cls_{args.samples}.csv", sep=",")
...
feature_layer = tf.keras.layers.Dense(1024, activation="relu")
...
checkpoint_dir_finetuning = "/scratch/hnkmah001/phd-projects/viewpoint-estimation-pascal/classification_multiclass_finetuning/{}_samples/checkpoints/".format(args.samples)

if not os.path.isdir(checkpoint_dir):
    os.mkdir(checkpoint_dir)
if not os.path.isdir(checkpoint_dir_finetuning):
    os.mkdir(checkpoint_dir_finetuning)
manager = tf.train.CheckpointManager(checkpoint, directory=checkpoint_dir, max_to_keep=args.patience+1)
manager_finetuning = tf.train.CheckpointManager(checkpoint, directory=checkpoint_dir_finetuning, max_to_keep=args.patience+1)

checkpoint.restore(manager.checkpoints[-1])
model.summary()

# Save logs with TensorBoard Summary
train_logdir = "/scratch/hnkmah001/phd-projects/viewpoint-estimation-pascal/classification_multiclass_finetuning/{}_samples/logs/train".format(args.samples)
val_logdir = "/scratch/hnkmah001/phd-projects/viewpoint-estimation-pascal/classification_multiclass_finetuning/{}_samples/logs/val".format(args.samples)
...
for epoch in range(args.epochs):
    ...
    ckpt_path = manager_finetuning.save()
